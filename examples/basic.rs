use dravya::{beam, elastic, material::Material, stress::StressTensor, yield_criteria};
...
fn main() {
    ...
    println!(
        "{}: E={:.0} GPa, σ_y={:.0} MPa",
        steel.name,
        steel.youngs_modulus / 1e9,
        steel.yield_strength / 1e6
    );
    ...
    println!(
        "Safety factor: {:.2}",
        yield_criteria::safety_factor(&s, steel.yield_strength)
    );
    ...
    println!(
        "Cantilever deflection (1kN, 1m, 100×10mm): {:.4} mm",
        delta * 1000.0
    );
}
